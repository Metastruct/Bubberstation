...
/datum/bodypart_overlay/mutant/tail
	...
	var/wag_timer_id
	/// Number of split animation frames for the current wag icon_state, populated by get_images(). 0 if not
	/// yet discovered, or permanently if this tail style has no split frames (falls back to the baked animation).
...
/datum/bodypart_overlay/mutant/tail/proc/get_wag_frame_time(mob/living/carbon/organ_owner)
	...
	var/datum/mood/mob_mood = organ_owner?.mob_mood
	if(mob_mood)
...
/// Sets the player's baseline wag speed (mood still scales on top of this). Takes effect on the next frame tick if already wagging, or the next time wagging starts otherwise.
...
/mob/living/carbon/human/verb/set_tail_wag_speed()
	...
	to_chat(src, span_notice("You adjust your tail's wagging speed to [wag_percent]% of normal."))
